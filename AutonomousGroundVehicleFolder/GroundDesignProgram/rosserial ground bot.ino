...
volatile int direction [] = {0,0,0,0};
volatile int speed [] = {0,0,0,0};
...
void messageCb( const std_msgs::Int16MultiArray& speed_msg){
  ...
  speed[0] = abs(speed_msg.data[0]);
  speed[1] = abs(speed_msg.data[1]);  
  speed[2] = abs(speed_msg.data[2]);
  speed[3] = abs(speed_msg.data[3]);

  direction[0] = speed_msg.data[0] >= 0 ? 1:-1;
  direction[1] = speed_msg.data[1] >= 0 ? 1:-1;
  direction[2] = speed_msg.data[2] >= 0 ? 1:-1;  
  direction[3] = speed_msg.data[3] >= 0 ? 1:-1;
}

ros::Subscriber<std_msgs::Int16MultiArray> sub("speed_msg", &messageCb );
...
void loop() {
  ...
  setMotor(direction[0], speed[0], PWM1, M1PIN1, M1PIN2);
